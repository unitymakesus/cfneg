 ...
 * We're changing the length of the excerpt and the "read more" text
 *
 */

class CFNEG_ET_Builder_Module_Blog extends ET_Builder_Module {
	function init() {
					...
					<article id="" <?php post_class( 'et_pb_post clearfix' . $no_thumb_class . $overlay_class ) ?>>
						...
						<?php if ( 'off' === $args['fullwidth'] || ! in_array( $post_format, array( 'link', 'audio', 'quote' ) ) ) { ?>
							...
							<?php
								...
								if ( 'on' === $args['show_content'] ) {
									...
									if ( has_excerpt() ) {
										...
										if ( '' !== $post_content ) {
											...
											$et_fb_processing_shortcode_object = false;
											echo et_delete_post_first_video( strip_shortcodes( truncate_post( 140, false, '', true ) ) ) ;
								...
								if ( 'on' !== $args['show_content'] ) {
									$more = 'on' == $args['show_more'] ? sprintf( ' <a href="%1$s" class="more-link" >%2$s</a>' , esc_url( get_permalink() ), esc_html__( 'Continue Reading', 'et_builder' ) )  : '';
									echo $more;
								}
								?>
		...
		if ( have_posts() ) {
			...
			<?php if ( 'off' === $fullwidth || ! in_array( $post_format, array( 'link', 'audio', 'quote' ) ) ) { ?>
				...
				<?php
					...
					if ( 'on' !== $show_content ) {
						$more = 'on' == $show_more ? sprintf( ' <a href="%1$s" class="more-link" >%2$s</a>' , esc_url( get_permalink() ), esc_html__( 'Continue Reading', 'et_builder' ) )  : '';
						echo $more;
					}
